...
check_vehicle.function <- function(vehicle, list_data, from){
  ...
  for (i in 1:length(list_data)){
    ...
    if(data_from == from && comp){
      ...
      #cat(vehicle, " is the vehicle in use\n")
      #cat(data_to, " is the current data_to\n")
      #cat(comp_check, "is current value in comp_check\n\n")
      
      if(vehicle == "Ambulance" && comp_check == FALSE){
        #cat(cat(data_to, " is the current data_to in inside if\n"))
        to <- data_to
        break
      } else { 
      to <- data_to 
...
time_calculator.function <- function(from_to, total_time_count, infusion_time_count, data, stop_sign, infusion_starts){
  ...
  if(data_from == from && data_to == to) { 
    ...
    cat(total_time_count, " is the total time that has now passed\n")
...
# Checking the vehicle again and edge (from_to) correctly
from <- current_stage[1]
from_to <- check_vehicle.function("Ambulance", list_data_a, from)

# Counting time until "Shock Room"
stop_sign <- "Shock Room"

current_stage <- time_calculator.function(from_to, as.numeric(current_stage[3]), as.numeric(current_stage[4]), list_data_a, stop_sign, infusion_starts)

cat("Patient transported to Shock Room!\n")
cat(as.numeric(current_stage[3]), "is the total time from the Risk Analysis\n")
cat(as.numeric(current_stage[4]), "is the total time from the Infusion Starts\n")
# At the moment the latter is from "Meeting the patient", though
